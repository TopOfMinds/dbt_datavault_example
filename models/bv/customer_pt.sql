...
WITH time_line AS (
  SELECT customer_key, effective_ts, 'customer_main_s' AS rec_src FROM  {{ ref('customer_main_s') }}
  UNION DISTINCT
  SELECT customer_key, effective_ts, 'customer_address_s' AS rec_src FROM  {{ ref('customer_address_s') }}
  UNION DISTINCT
  SELECT customer_key, effective_ts, 'customer_class_sb' AS rec_src FROM  {{ ref('customer_class_sb') }}
), cms AS (
  SELECT
    customer_key
    ,load_dts
...
FROM
  ...
  LEFT JOIN  {{ ref('customer_class_sb') }} ccs ON ccs.customer_key = tl.customer_key AND ccs.effective_ts <= tl.effective_ts AND tl.effective_ts < ccs.effective_ts_end
